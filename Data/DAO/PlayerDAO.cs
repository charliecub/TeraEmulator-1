...
namespace Data.DAO
{
    public class PlayerDAO : BaseDAO
    {
        ...
        {
            string cmdString = "SELECT * FROM players WHERE AccountName=?username";
            ...
            {
                cmdString = "SELECT * FROM player_datas WHERE PlayerId=?id";
        ...
        public int SaveNewPlayer(Player player)
        {
            ...
            + "VALUES (?accname, ?lvl, ?exp, ?exprev, ?mount, ?uiset, ?gaccept, ?praisgive, ?lastpraise, ?bank, ?credate); SELECT LAST_INSERT_ID();";
            MySqlCommand command = new MySqlCommand(cmdString, PlayerDAOConnection);
        ...
        {
            string cmdString = "UPDATE players SET"
            ...
            {
                Log.ErrorException("UpdatePlayer Error", ex);
                return;
            }

            cmdString = "UPDATE player_datas SET "
            ...
            {
                Log.ErrorException("UPDATE player_datas Error", ex);
                return;
            }
        }
    }
